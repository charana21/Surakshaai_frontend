...
import { Upload, FileSpreadsheet, CheckCircle, AlertTriangle, Loader2, BarChart3 } from 'lucide-react';
...
export function UtsUpload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await calendarApi.uploadUtsData(file);
      setResult(response);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Upload UTS & PRS Booking Data</DialogTitle>
          <DialogDescription>
            Upload an Excel file to update historical UTS and PRS booking records.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          <div className="space-y-4">
            <div className="grid w-full items-center gap-2">
              <Label htmlFor="uts-file" className="text-sm font-medium">Select Excel File (.xlsx, .xls)</Label>
              <div 
                className={cn(
              ...
              >
                ...
                {file ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-muted-foreground/70 text-center">Required columns: DATE, UTS INWARD, UTS OUTWARD, UTS TOTAL, PRS INWARD, PRS OUTWARD, PRS TOTAL, OVERALL TOTAL</p>
                  </>
                )}
              </div>
            </div>
